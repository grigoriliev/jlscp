...
public class Client {
	...
	toEscapedText(String s) {
		s = Parser.toEscapedString(s);
		return conv(s);
	}
	
	private String
	toEscapedFsEntry(String s) {
		s = Parser.toEscapedFileName(s);
		return conv(s);
	}
	...
	private String
	conv(String s) {
		return getExtendedCharacterEscaping() ? Parser.toExtendedEscapeSequence(s) : s;
	}
	...
	fireDeviceMidiDataEvent(String s) {
		try {
			String[] list = Parser.parseList(s, ' ');
			if(list.length != 5) {
				getLogger().warning("Unknown DEVICE_MIDI format");
				return;
			}
			
			int dev = Parser.parseInt(list[0]);
			int port = Parser.parseInt(list[1]);
			
			MidiDataEvent.Type type = parseMidiDataType(list[2]);
			if(type == null) return;
			
			int note = Parser.parseInt(list[3]);
			int velocity = Parser.parseInt(list[4]);
	...
	fireChannelMidiDataEvent(String s) {
		try {
			...
			int channel = Parser.parseInt(list[0]);
			
			MidiDataEvent.Type type = parseMidiDataType(list[1]);
			if(type == null) return;
			
			int note = Parser.parseInt(list[2]);
			int velocity = Parser.parseInt(list[3]);
		 ...
		 if(s.startsWith("CHANNEL_MIDI:")) {
			...
			if(s.startsWith("NAME ")) {
				...
				try {
					...
					list = Parser.parseEscapedStringList(s, ' ');
					if(list.length != 2) throw new LscpException();
					list[1] = Parser.toNonEscapedString(list[1]);
			...
			try {
				...
				Integer[] i = Parser.parseIntList(s, ' ');
				if(i.length != 2) {
	...
	getAudioOutputDriverParameterInfo(String driver, String param, Parameter... deplist)
		...
		ParameterType type = Parser.parseType(lnS);
		boolean multi = Parser.parseMultiplicity(lnS);
		Parameter prm;
	...
	getAudioOutputDeviceInfo(int deviceId) throws IOException, LscpException, LSException {
		...
		String drv = Parser.getCategoryInfo(lnS, "DRIVER");
	...
	getAudioOutputChannelInfo(int deviceId, int audioChn)
		...
		for(String s : lnS) {
			if(s.startsWith("NAME: ")) {
				...
				prm.setValue(Parser.removeQuotation(s));
				...
				prm.setValue(Parser.parseInt(s));
	...
	getMidiInstrumentMapInfo(int mapId) throws IOException, LscpException, LSException {
		...
		for(String s : lnS) {
			if(s.startsWith("NAME: ")) {
				name = Parser.toNonEscapedString(s.substring("NAME: ".length()));
	...
	getMidiInstruments() throws IOException, LscpException, LSException {
		...
		String[] entries = Parser.parseArray(getSingleLineResultSet().getResult());
	...
	getMidiInstrumentEntries(int mapId) throws IOException, LscpException, LSException {
		...
		for(int i = 0; i < entries.length; i++) {
			Integer[] vals = Parser.parseIntList(entries[i]);
			if(vals.length != 3) {
	...
	getMidiInstruments(String[] entries) throws IOException, LscpException, LSException {
		...
		for(String s : entries) {
			Integer[] vals = Parser.parseIntList(s);
			if(vals.length != 3) {
				throw new LscpException(LscpI18n.getLogMsg("CommandFailed!"));
			}
	...
	getChannelStreamCount(int samplerChn) throws IOException, LscpException, LSException {
		...
		return Parser.parseInt(rs.getResult());
	}
	...
	getChannelBufferFillBytes(int samplerChn) throws IOException, LscpException, LSException {
		...
		String[] args = Parser.parseList(rs.getResult());
		...
		for(String s : args) {
			...
			bf.setStreamId(Parser.parseInt(s.substring(1, i)));
			bf.setValue(Parser.parseInt(s.substring(i + 1)));
			v.add(bf);
		}
	...
	getChannelBufferFillPercentage(String ln) throws LscpException {
		...
		Vector<BufferFill> v = new Vector<BufferFill>();
		String[] args = Parser.parseList(ln);
		...
		for(String s : args) {
			...
			BufferFill bf = new BufferFill();
			bf.setStreamId(Parser.parseInt(s.substring(1, i)));
			bf.setValue(Parser.parseInt(s.substring(i + 1, s.length() - 1)));
			v.add(bf);
		}
				...
				throws IOException, LscpException, LSException
	{
		...
		for(String s : rs.getMultiLineResult()) {
			if(s.startsWith("EFFECT_SEQUENCE: ")) {
				...
				Integer[] eis = Parser.parseIntList(s);
	...
	getDbDirectoryNames(String dir) throws IOException, LscpException, LSException {
		...
		String[] names = Parser.parseEscapedStringList(getSingleLineResultSet().getResult());
		for(int i = 0; i < names.length; i++) {
			names[i] = Parser.toNonEscapedString(names[i]);
		}
		return names;
	}
	...
	getDbDirectoryInfo(String dir) throws IOException, LscpException, LSException {
		...
		if(dir.equals("/")) {
			...
			dir = Parser.removeEndingFileSeparator(dir);
		}
		String s = Parser.getFileName(dir);
		if(s != null) info.setName(Parser.toNonEscapedFileName(s));
		s = Parser.getParentDirectory(dir);
	...
	getDbDirectories(String dir) throws IOException, LscpException, LSException {
		...
		if(!Parser.hasEndingFileSeparator(dir)) dir += "/";
	...
	getDbInstrumentInfo(String instr) throws IOException, LscpException, LSException {
		...
		String s = Parser.getParentDirectory(instr);
		if(s != null) info.setDirectoryPath(s);
		s = Parser.getFileName(instr);
		if(s != null) info.setName(Parser.toNonEscapedFileName(s));
	...
	findDbDirectories(String dir, DbSearchQuery query, boolean nonRecursive)
		...
		String[] dirS = Parser.parseEscapedStringList(getSingleLineResultSet().getResult());
	...
	findDbInstruments(String dir, DbSearchQuery query, boolean nonRecursive)
		...
		String[] instrS = Parser.parseEscapedStringList(getSingleLineResultSet().getResult());
	...
	getLogger() { return Logger.getLogger("com.grigoriliev.jsampler.jlscp"); }
}
